...
var KnackApp = function ($, info = {}) {
    ...
    function chznBetterSetFocus() {
        try {
            ...
            //Add year to Year_Batch selector for Dispenser Terminal.
            if (fieldId === 'field_1173') {
                var chznBetterTxt = $('#chznBetter').val();
                var year = new Date().getFullYear() + '_';
                if (!chznBetterTxt.startsWith(year)) {
                    $('#chznBetter').val(year + chznBetterTxt);
                }
            }
        }
        catch (e) {
